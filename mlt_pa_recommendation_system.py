...
dengan Nilai 'Sci-Fi' diganti dengan 'Scifi' menggunakan `regex`
"""
...
"""Setelah menyelesaikan pelatihan model Collaborative Filtering, hasil evaluasi pada data uji menunjukkan kinerja yang memuaskan. Fungsi kerugian (Test Loss) sebesar 0.5432 mencerminkan seberapa baik model memberikan prediksi terhadap nilai sebenarnya, dengan nilai yang relatif rendah menandakan tingkat akurasi yang baik. Selain itu, Root Mean Squared Error (Test RMSE) sebesar 0.1442 mengindikasikan deviasi rata-rata antara prediksi dan nilai sebenarnya, dengan nilai yang rendah menunjukkan tingkat akurasi yang tinggi dalam memprediksi preferensi pengguna terhadap item. Dengan hasil evaluasi tersebut, dapat disimpulkan bahwa model Collaborative Filtering telah berhasil dalam tugas rekomendasi item dan memiliki kemampuan prediktif yang baik pada data uji.
...
"""Metrik Evaluasi menunjukkan bahwa model memiliki performa yang baik dan model konvergen pada epochs sekitar 100. Dari proses ini, kita memperoleh nilai error akhir sebesar sekitar 0.0861 dan error pada data validasi sebesar 0.2628. Nilai tersebut dapat digunakan untuk membuat sistem rekomendasi.
